...
### Mode A: Claude Code (CLI / VS Code)
Generate 7 separate markdown parts using **parallel subagents**, then merge into a `.docx` using the bundled Python script. See [output-format.md](references/output-format.md) for the full part structure and generation process.

#### Parallel Generation with Subagents
Once the Design Decisions Document is written to `Design_Decisions.md`, use the **Agent tool** to generate parts in parallel. Each subagent runs independently with its own context — no token competition.

**Launch sequence:**

**Batch 1 — 3 agents in parallel:**
- Agent A → `Part1_Executive_Architecture_ISO.md`
- Agent B → `Part2_Network_Wireless_Server.md`
- Agent C → `Part3_DR_Monitoring_Power.md`

**Batch 2 — 3 agents in parallel (after Batch 1 completes):**
- Agent D → `Part4_BOM_Assets_Timeline.md`
- Agent E → `Part5_Security_Stress_Test.md`
- Agent F → `Part6_SOPs.md` (all 11 SOPs in one file for DOCX mode)

**Each subagent prompt must include:**
1. The full content of `Design_Decisions.md`
2. The part assignment with every section and table it must contain
3. Diagram requirements for that part (from output-format.md)
4. The zero compression rule: no placeholders, no abbreviations, every row populated
5. Instruction to write output to the specific filename

**After all subagents complete**, the main agent:
1. Reads each generated part file
2. Runs completeness verification (count check, no placeholders, diagrams present)
3. Fixes any gaps
4. Merges into `.docx` using `scripts/generate_docx.py`

This cuts document generation time from ~30 minutes (sequential) to ~10 minutes (2 parallel batches).
...
1. Read `memory/memory_index.md` to check for returning clients or relevant past projects
2. If the client exists in `memory/clients/`, load their profile to pre-fill known preferences, non-negotiables, and vendor history
3. Check `memory/pricing/` for recent pricing data to generate more accurate BOMs
4. Check `memory/lessons/` for relevant lessons from similar projects
...
3. **Design Decisions Document** — Generate a standalone summary of ALL confirmed requirements, specs, vendor choices, and design decisions
4. **Document Writing:**
   - **Claude Code:** Launch 2 batches of 3 parallel subagents (each writes one `.md` part file)
   - **Browser:** Generate all 7 HTML artifacts back-to-back without pausing between parts
5. **Security Stress Test** — Red-team the finalized design (Part 5 — generated as part of step 4)
6. **Review** — User reviews all parts at once, requests changes to specific parts only
7. **Final Combined Output:**
   - **Claude Code:** Merge all 6 `.md` parts into one `.docx` via `scripts/generate_docx.py`
   - **Browser:** Automatically generate one combined HTML artifact containing all parts — do NOT require the user to merge manually
...
For detailed reference on standards and compliance, see [iso-27001-controls.md](references/iso-27001-controls.md).
For equipment reference guides, see [vendor-reference.md](references/vendor-reference.md).
For implementation plan template (markdown), see [plan-template.md](templates/plan-template.md).
For HTML output template, see [implementation-plan.html](templates/implementation-plan.html).
